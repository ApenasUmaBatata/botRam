  ...
  run: async (bot, message, args) => {
    if (!args[0]) {
      embed = new MessageEmbed()
        ...
        .setFooter(`Digite ' ${prefix}ação <CommandName> <Menção>' para que ocorra uma ação!`, message.author.avatarURL())
        .setColor("#ff00c3");
      message.reply({ embeds: [embed] });
    }
    //Mencione alguém para que ocorra uma ação!
    if (args[0] == "abraçar") {
      if (!message.mentions.members.first()) { //puxando a menção do autor
        ...
        const embed = new MessageEmbed() //criando uma embed
          .setDescription(`${message.author} ${fr} ${message.mentions.members.first()}`)
    ...
    if (args[0] == "tapa") {
      if (!message.mentions.members.first()) { //puxano a menção do autor
        message.reply("Mencione alguém para que ocorra uma ação!")
      } else {
        var image = [
